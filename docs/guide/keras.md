...
will work as expected. It does not yet work with Sparse Features for Feature
Columns.
...
## Native Keras (without `model_to_estimator`)
...
will use the CPU if no GPUs are found. To verify it actually uses GPU, enable
info level tensorflow logging:
